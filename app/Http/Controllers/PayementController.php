    ...
    public function initPayement()
    {
        ...
        foreach ($employers as $employer) {
            ...
            if (!$aEtePaye) {
                ...
                $payment->launch_date = Carbon::now(); // Date du paiement
                $payment->done_time = Carbon::now(); // Date de confirmation du paiement
                $payment->status = 'SUCCESS'; // Statut du paiement
                $payment->month = $currentMonthFrench; // Mois en français
                $payment->year = $currentYear; // Année
                $payment->save();
            }
        }
